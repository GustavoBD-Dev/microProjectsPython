...
status = np.zeros((nX, nY)) # Initialice the status of cells 
    ...
    for x in range(0, nX):
        for y in range(0, nY):
            ...
            if not pauseRun:
                ...
                # REGLA 1: Una celula muerta con tres celulas vivas revive 
                if status[x, y] == 0 and nNeigh == 3:
                    newStatus[x, y] = 1

                # REGLA 2: Una celula viva con mas de 3 vecinos o menos de 2muere 
                elif status[x, y] == 1 and (nNeigh < 2 or nNeigh > 3):
                    newStatus[x, y] = 0

            poly = [(x*xSize, y*ySize), ((x+1)*xSize, y*ySize),
                    ((x+1)*xSize, (y+1)*ySize), (x*xSize, (y+1)*ySize)]
